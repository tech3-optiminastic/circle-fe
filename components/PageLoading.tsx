  ...
  return (
    <div className="space-y-5 animate-pulse" aria-busy="true" aria-label="Loading">
      ...
      <div className="space-y-2">
        <div className="h-4 w-56 bg-[#ECEDF0] rounded" />
        <div className="h-3 w-80 max-w-full bg-[#EDEEF1] rounded" />
      </div>

      {/* Stat cards */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="bg-[#FFFFFF] border border-[#E4E6EA] rounded-xl px-4 py-3 space-y-2">
            <div className="h-2.5 w-20 bg-[#EDEEF1] rounded" />
            <div className="h-6 w-10 bg-[#ECEDF0] rounded" />
          </div>
        ))}
      </div>
      ...
      <div className="bg-[#FFFFFF] border border-[#E4E6EA] rounded-xl p-4 space-y-3">
        {Array.from({ length: 6 }).map((_, i) => (
          <div key={i} className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-full bg-[#EDEEF1] shrink-0" />
            <div className="flex-1 space-y-1.5">
              <div className="h-3 bg-[#ECEDF0] rounded" style={{ width: `${70 - i * 6}%` }} />
              <div className="h-2.5 bg-[#EDEEF1] rounded" style={{ width: `${45 - i * 4}%` }} />
            </div>
            <div className="h-5 w-16 bg-[#EDEEF1] rounded-full shrink-0" />
          </div>
        ))}
      </div>
    </div>
