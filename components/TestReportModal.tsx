...
export function TestReportModal({ invite, onClose }: { invite: TestInvite; onClose: () => void }) {
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-2xl border border-[#E1D6BC] shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col"
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-start justify-between gap-3 px-6 py-4 border-b border-[#EAE1CC]">
        ...
        <div className="overflow-y-auto px-6 py-5 space-y-5">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-2.5">
            {stats.map(s => (
              <div
                key={s.label}
                className="bg-[#F7F1E4] border border-[#E1D6BC] rounded-xl px-3 py-2.5 text-center"
              >
